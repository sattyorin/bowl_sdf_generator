...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--height", type=float, required=True, help="height of the cooking_bowl"
    )
    parser.add_argument(
        "--thickness", type=float, required=True, help="thickness of the cooking_bowl"
    )
    parser.add_argument(
        "-o",
        "--out_dir",
        default=".",
        required=True,
        help="thickness of the cooking_bowl",
    )
...
class BowlGenerator:
    ...
    def gen_bottom_plate_stl(self):
        ...
        model_mesh.save(os.path.join(self.out_dir, "meshes", "cooking_bowl_bottom.stl"))
    ...
    def gen_side_fragment_stl(self):
        ...
        model_mesh.save(
            os.path.join(self.out_dir, "meshes", "cooking_bowl_fragment.stl")
        )

    def gen_model_config(self):
        CONTENTS = """<?xml version = "1.0"?>
<model>
  <name>cooking_bowl</name>
  <version>1.0</version>
...
"""
    ...
    def gen_model_sdf(self):
        fragments = [
            "\n".join(
                [
                    ...
                    "        <geometry>",
                    "          <mesh>",
                    "            <uri>model://cooking_bowl/meshes/cooking_bowl_fragment.stl</uri>",
                    "          </mesh>",
                    "        </geometry>",
                    "      </collision>",
                    '      <visual name="visual">',
        ...
        lines = [
            ...
            "        <geometry>",
            "          <mesh>",
            "            <uri>model://cooking_bowl/meshes/cooking_bowl_bottom.stl</uri>",
            "          </mesh>",
            "        </geometry>",
            "      </collision>",
            '      <visual name="visual">',
